...
const SalesForecastTable: React.FC<SalesForecastTableProps> = ({
  header,
  rows,
  handleEditPrediction,
}) => {
          ...
          <thead>
            ...
            <tr className="bg-blue-900 text-white">
              {header.months.map((m, i) => (
                <th
                  key={i}
                  className="p-2 border border-gray-300 min-w-[100px]"
                >
                  {m}
                </th>
              ))}
          ...
          <tbody>
            {editableRows.map((row, rowIndex) => {
              ...
              return (
                ...
                >
                  {/* col แรก */}
                  <th className="sticky left-0 z-20 bg-white p-2 text-left font-medium border border-gray-300 w-[100px] min-w-[100px] max-w-[100px] sm:w-[200px] sm:min-w-[200px] sm:max-w-[200px]">
                    <span className="block lg:hidden text-xs font-normal">
                      {row.label}
                    </span>
                  ...
                  {/* col สอง */}
                  <td className="sticky left-[99px] z-10 bg-white p-2 text-right border border-gray-300 w-[90px] min-w-[90px] max-w-[90px] sm:left-[200px] sm:w-[160px] sm:min-w-[160px] sm:max-w-[160px]">
                  ...
                  {row.values.slice(1).map((value, valueIndex) => (
                    <td
                      key={valueIndex}
                      className="p-0 h-[35px] border border-gray-300"
                    >
                      {editMode && isHighlight ? (
                        <InputAction
                          placeholder=""
