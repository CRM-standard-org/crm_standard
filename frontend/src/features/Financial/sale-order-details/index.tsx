...
import {
  TypeSaleOrderPaymentFileResponse,
  TypeSaleOrderPaymentLog,
  TypeSaleOrderProducts,
  TypeSaleOrderResponse,
} from "@/types/response/response.saleorder";
...
export default function SaleOrderDetails() {
  ...
  const [isRejectDealDialogOpen, setIsRejectDealDialogOpen] =
    useState<boolean>(false);
  const [isCloseSaleDialogOpen, setIsCloseSaleDialogOpen] =
    useState<boolean>(false);

  const [dataSaleOrder, setDataSaleOrder] = useState<TypeSaleOrderResponse>();
  const [selectedPaymentFiles, setSelectedPaymentFiles] = useState<
    { payment_file_url: string }[]
  >([]);

  //หาสถานะล่าสุด
  const latestStatus =
    dataSaleOrder?.status?.[dataSaleOrder.status.length - 1].sale_order_status;
  ...
  const [saleOrderPaymentFile, setSaleOrderPaymentFile] =
    useState<TypeSaleOrderPaymentFileResponse>();
  ...
  const { data: saleOrderDetails, refetch: refetchSaleOrder } =
    useSaleOrderById({ saleOrderId });
  useEffect(() => {
      ...
      const formattedData = dataSaleOrder?.sale_order_product.map(
        (item: TypeSaleOrderProducts, index: number) => ({
          ...
          cells: [
            ...
            {
              value: item.group_product.group_product_name ?? "-",
              className: "text-center",
            },
            { value: item.unit.unit_name ?? "-", className: "text-center" },
            {
              value: Number(item.unit_price).toFixed(2).toLocaleString() ?? 0,
              className: "text-right",
            },
            {
              value:
                Number(item.sale_order_item_count)
                  .toFixed(2)
                  .toLocaleString() ?? 0,
              className: "text-right",
            },
            {
              value:
                Number(item.unit_discount).toFixed(2).toLocaleString() ?? 0,
              className: "text-right",
            },
            {
              value:
                Number(item.unit_discount_percent)
                  .toFixed(2)
                  .toLocaleString() ?? 0,
              className: "text-right",
            },
            {
              value:
                Number(item.sale_order_item_price)
                  .toFixed(2)
                  .toLocaleString() ?? 0,
              className: "text-right",
            },
      ...
      const formattedData = dataSaleOrder?.sale_order_payment_log.map(
        (item: TypeSaleOrderPaymentLog, index: number) => ({
          ...
          cells: [
            { value: index + 1, className: "text-center" },
            {
              value: new Date(item.payment_date).toLocaleDateString("th-TH"),
              className: "text-center",
            },
            {
              value: Number(item.amount_paid).toFixed(2).toLocaleString() ?? 0,
              className: "text-right",
            },
            { value: item.payment_term_name ?? "-", className: "text-center" },
            {
              value: item.payment_method.payment_method_name ?? "-",
              className: "text-center",
            },
  ...
  const {
    ...
    return {
      latestExpectManufactureDate: getLatestFieldDate(
        dataSaleOrder.status,
        "expected_manufacture_factory_date"
      ),
      latestManufactureDate: getLatestFieldDate(
        dataSaleOrder.status,
        "manufacture_factory_date"
      ),
      latestExpectDeliveryDate: getLatestFieldDate(
        dataSaleOrder.status,
        "expected_delivery_date_success"
      ),
      latestDeliveryDate: getLatestFieldDate(
        dataSaleOrder.status,
        "delivery_date_success"
      ),
      latestExpectReceiptDate: getLatestFieldDate(
        dataSaleOrder.status,
        "expected_receipt_date"
      ),
      latestReceiptDate: getLatestFieldDate(
        dataSaleOrder.status,
        "receipt_date"
      ),
    };
  ...
  const { data: dataPaymentFile, refetch: refetchPaymentFile } =
    usePaymentFileById({ paymentLogId });
  ...
  const remainingTotal =
    dataSaleOrder?.grand_total - dataSaleOrder?.totalAmountPaid;
      ...
      <div className="flex mb-3 justify-between">
        ...
        <div className="flex gap-2">
          {latestStatus && !["สำเร็จ", "ไม่สำเร็จ"].includes(latestStatus) && (
            <Link to={`/edit-sale-order/${saleOrderId}`}>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <div className="">
                <LabelWithValue
                  label="ลูกค้า"
                  value={`${dataSaleOrder?.customer.company_name}`}
              ...
              <div className="flex flex-row ">
                <label className="whitespace-nowrap 2xl:me-28">
                  ความสำคัญ<span style={{ color: "red" }}>*</span>
                </label>
                <RatingShow
                  value={dataSaleOrder?.priority ?? 0}
                  className="2xl:ms-14 w-6 h-6"
                />
              </div>
                ...
                <LabelWithValue
                  ...
                  value={
                    dataSaleOrder?.issue_date
                      ? new Date(dataSaleOrder.issue_date).toLocaleDateString(
                          "th-TH"
                        )
                      : "-"
                  }
              ...
              <div className="">
                <LabelWithValue
                  ...
                  value={`${dataSaleOrder?.customer.tax_id}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="">
                <LabelWithValue
                  label="ผู้รับผิดชอบ"
                  value={`${dataSaleOrder?.responsible.first_name} ${dataSaleOrder?.responsible.last_name}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
                ...
                <LabelWithValue
                  ...
                  value={
                    dataSaleOrder?.price_date
                      ? new Date(dataSaleOrder.price_date).toLocaleDateString(
                          "th-TH"
                        )
                      : "-"
                  }
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <div className="">
                <LabelWithValue
                  label="การรับสินค้า"
                  value={`${dataSaleOrder?.shipping_method}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
              ...
              <div className="">
                <LabelWithValue
                  label="ชื่อสถานที่"
                  value={`${dataSaleOrder?.place_name}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
            ...
            <div className="space-y-4">
              {dataSaleOrder?.shipping_remark && (
                <div className="">
                  <LabelWithValue
                    ...
                    value={`${dataSaleOrder?.shipping_remark}`}
                    classNameLabel="sm:w-1/2"
                    classNameValue="w-80"
                  />
                </div>
              ...
              <div className="">
                <LabelWithValue
                  label="ประเภท"
                  value={`${dataSaleOrder?.country.country_name}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
              ...
              <div className="">
                <LabelWithValue
                  label="จังหวัด"
                  value={`${dataSaleOrder?.province.province_name}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
              ...
              <div className="">
                <LabelWithValue
                  label="อำเภอ"
                  value={`${dataSaleOrder?.district.district_name}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <div className="">
                <LabelWithValue
                  label="ชื่อผู้ติดต่อ"
                  value={`${dataSaleOrder?.contact_name}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
              ...
              <div className="">
                <LabelWithValue
                  ...
                  value={`${dataSaleOrder?.contact_email}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
            ...
            <div className="space-y-4">
              <div className="">
                <LabelWithValue
                  ...
                  value={`${dataSaleOrder?.contact_phone}`}
                  classNameLabel="sm:w-1/2"
                  classNameValue="w-80"
                />
              </div>
          ...
          <h1 className="text-xl font-semibold mb-1">
            ...
            {remainingTotal === 0 ? (
              ...
              <span className="text-red-500">
                ยอดค้างชำระ {remainingTotal.toFixed(2).toLocaleString()} บาท
              </span>
            )}
            )
          </h1>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <div className="flex justify-between space-x-4">
                <label>ราคารวม</label>
                <label>
                  {Number(dataSaleOrder?.total_amount)
                    .toFixed(2)
                    .toLocaleString()}
                </label>
              </div>
              ...
              <div className="flex justify-between space-x-4">
                <label>
                  ...
                  <span className="text-main">
                    {" "}
                    {Number(dataSaleOrder?.special_discount)
                      .toFixed(2)
                      .toLocaleString()}{" "}
                    บาท{" "}
                  </span>
                ...
                <label>
                  {Number(dataSaleOrder?.amount_after_discount)
                    .toFixed(2)
                    .toLocaleString()}
                </label>
              </div>
              ...
              <div className="flex justify-between space-x-4">
                <label>VAT (%)</label>
                <label>
                  {Number(dataSaleOrder?.vat_amount)
                    .toFixed(2)
                    .toLocaleString()}
                </label>
              </div>
              ...
              <div>
                ...
                <p className="mt-2 text-blue-600">
                  {dataSaleOrder?.additional_notes ?? "-"}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between space-x-4">
                ...
                {dataSaleOrder?.payment_term_name === "เต็มจำนวน" && (
                  <div className="flex flex-row space-x-5">
                    <p className="text-blue-600">
                      {dataSaleOrder?.payment_term_name}
                    </p>
                    <label>ภายใน</label>
                    <p className="text-blue-600">
                      {dataSaleOrder?.payment_term_day}
                    </p>
                ...
                {dataSaleOrder?.payment_term_name === "แบ่งชำระ" && (
                  <div className="flex flex-row space-x-5">
                    ...
                    <label>ภายใน</label>
                    <p className="text-blue-600">
                      {dataSaleOrder?.payment_term_installment}
                    </p>
                ...
                <>
                  <div className="mt-4 border border-gray-300 rounded-xl p-4 bg-white shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-4">
                      {dataSaleOrder?.sale_order_payment_term?.map(
                        (term, index) => (
                          <div
                            key={term.payment_term_id || index}
              ...
              <div className="flex justify-between space-x-4">
                ...
                <label className="text-blue-600">
                  {dataSaleOrder?.payment_method.payment_method_name}
                </label>
              </div>
              <div className="flex justify-between space-x-4">
                <label>สกุลเงิน</label>
                <label className="text-blue-600">
                  {dataSaleOrder?.currency.currency_name}
                </label>
              </div>
              ...
              <div className="flex justify-between space-x-4">
                ...
                <label className="text-blue-600">
                  {dataSaleOrder?.remark ?? "ไม่มีหมายเหตุ"}
                </label>
              </div>
              <div className="">
                ...
                <div className="flex overflow-x-auto gap-4 mt-2 pb-2">
                  {saleOrderDetails?.responseObject?.sale_order_file?.map(
                    (file, index) => {
                      const fileUrl = `${appConfig.baseApi}${file.sale_order_file_url}`;
                      const isPdf = file.sale_order_file_url
                        .toLowerCase()
                        .endsWith(".pdf");
              ...
              <div className="flex items-start gap-3 mb-5">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <LabelWithValue
                    ...
                    value={
                      latestManufactureDate
                        ? new Date(latestManufactureDate).toLocaleDateString(
                            "th-TH"
                          )
                        : "-"
                    }
              ...
              <div className="flex items-start gap-3 mb-5">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <LabelWithValue
                    ...
                    value={
                      latestExpectDeliveryDate
                        ? new Date(latestExpectDeliveryDate).toLocaleDateString(
                            "th-TH"
                          )
                        : "-"
                    }
                  ...
                  <LabelWithValue
                    ...
                    value={
                      latestDeliveryDate
                        ? new Date(latestDeliveryDate).toLocaleDateString(
                            "th-TH"
                          )
                        : "-"
                    }
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <LabelWithValue
                    ...
                    value={
                      latestExpectReceiptDate
                        ? new Date(latestExpectReceiptDate).toLocaleDateString(
                            "th-TH"
                          )
                        : "-"
                    }
                  ...
                  <LabelWithValue
                    ...
                    value={
                      latestReceiptDate
                        ? new Date(latestReceiptDate).toLocaleDateString(
                            "th-TH"
                          )
                        : "-"
                    }
              ...
              <div className="max-h-[620px] overflow-y-auto pr-2 space-y-4">
                {dataSaleOrder?.status.map((status, index) => {
                  const getStatusColor = (statusName) => {
                    if (statusName.includes("การผลิต"))
                      return "bg-green-500 text-white";
                    if (statusName.includes("ขนส่ง"))
                      return "bg-sky-400 text-white";
                    if (statusName.includes("ได้รับ"))
                  ...
                  } ${status.created_by_employee.last_name || ""}`.trim();
                  ...
                  return (
                    <div key={index} className="flex items-start gap-3">
                      ...
                      <div className="flex-1 space-y-2">
                        <div
                          className={`${labelColor}  text-sm px-2 py-1 rounded-md inline-block`}
                        >
                          สถานะใบสั่งขาย:{" "}
                        ...
                        {["สำเร็จ", "ไม่สำเร็จ"].includes(
                          status.sale_order_status
                        ) && (
                          <div className="text-sm text-slate-700">
                            หมายเหตุ:{" "}
                            <span className="font-semibold">
                              {status.sale_order_status_remark || "-"}
                            </span>
                          </div>
                        )}
                      </div>
        ...
        <div className="flex overflow-x-auto gap-4 mt-2 pb-2">
          {saleOrderPaymentFile?.payment_file?.length > 0 ? (
            saleOrderPaymentFile.payment_file.map((file, index) => {
              ...
              const isPdf = file.payment_file_url
                .toLowerCase()
                .endsWith(".pdf");
