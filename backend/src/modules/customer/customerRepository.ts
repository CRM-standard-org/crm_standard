...
export const customerRepository = {
  ...
  mainCustomerContact: async (
    ...
    await prisma.customerContact.updateMany({
      where: {
        customer_id: cust_id,
        NOT: { customer_contact_id: cust_contact_id },
      },
      data: { main: false },
    });
  ...
  updateCustomerContact: async (
    ...
    const checkSocial = await prisma.detailSocial.findFirst({
      where: { customer_contact_id: setForm.customer_contact_id },
    });

    const checkCharacter = await prisma.customerCharacter.findFirst({
      where: { customer_contact_id: setForm.customer_contact_id },
    });
  ...
  deleteCustContact: async (cust_id: string, cust_contact_id: string) => {
    ...
    const detailSocial = await prisma.detailSocial.findFirst({
      where: { customer_contact_id: custContact.customer_contact_id },
    });
    const customerCharacter = await prisma.customerCharacter.findFirst({
      where: { customer_contact_id: custContact.customer_contact_id },
    });

    if (detailSocial) {
      await prisma.customerCharacter.deleteMany({
        where: { customer_contact_id: cust_contact_id },
      });
    }
  ...
  followQuotation: async (customer_id: string) => {
    ...
    const result = await prisma.quotation.aggregate({
      where: {
        customer_id,
        NOT: { quotation_status: { in: ["ยกเลิก", "สำเร็จ", "ไม่สำเร็จ"] } },
      },
