...
import {
  ...
  TypePayloadContact,
  TypePayloadCompany,
} from "@modules/customer/customerModel";
import { number } from "zod";

export const customerService = {
  create: async (payload: TypePayloadCustomer, employee_id: string) => {
    try {
      const checkCust = await customerRepository.findByname(
  ...
  delete: async (customer_id: string) => {
    try {
      ...
      return new ServiceResponse(
        ResponseStatus.Failed,
        (ex as any).code === "P2003"
          ? "Deletion failed: this data is still in use"
          : errorMessage,
        null,
        StatusCodes.INTERNAL_SERVER_ERROR
      );
  ...
  addCustContact: async (
    ...
    try {
      ...
      const errorMessage =
        "Error add customer contact :" + (ex as Error).message;
      return new ServiceResponse(
        ResponseStatus.Failed,
        errorMessage,
        null,
  ...
  addAddress: async (
    ...
    try {
      await customerRepository.addAddress(cust_id, payload, employee_id);
      return new ServiceResponse(
        ResponseStatus.Success,
  ...
  mainCustContact: async (
    cust_id: string,
    cust_contact_id: string,
    employee_id: string
  ) => {
    try {
      const data = await customerRepository.mainCustomerContact(
        cust_id,
        cust_contact_id,
        employee_id
      );
      ...
      const errorMessage =
        "Error Change main customer contact :" + (ex as Error).message;
      return new ServiceResponse(
        ResponseStatus.Failed,
        errorMessage,
        null,
  ...
  mainAddress: async (
    ...
    try {
      const data = await customerRepository.mainAddress(
        cust_id,
        address_id,
        employee_id
      );
      ...
      const errorMessage =
        "Error Change main address :" + (ex as Error).message;
      return new ServiceResponse(
        ResponseStatus.Failed,
        errorMessage,
        null,
  ...
  updateCustContact: async (
    cust_id: string,
    payload: TypePayloadContact,
    employee_id: string
  ) => {
    try {
      const checkCustContact = await customerRepository.findByIdCustContact(
        cust_id,
      ...
      const errorMessage =
        "Error update customer contact :" + (ex as Error).message;
      return new ServiceResponse(
        ResponseStatus.Failed,
        errorMessage,
        null,
  ...
  updateAddress: async (
    ...
    try {
      const checkAddress = await customerRepository.findByIdAddress(
        cust_id,
        payload.address_id
      );
      if (!checkAddress) {
      ...
      const { place_name, address, country_id, province_id, district_id } = {
        ...checkAddress,
        ...payload,
  ...
  deleteCustContact: async (cust_id: string, cust_contact_id: string) => {
    try {
      ...
      const custContact = await customerRepository.deleteCustContact(
        cust_id,
        cust_contact_id
      );
      if (custContact === null) {
      ...
      const errorMessage =
        "Error delete customer contact :" + (ex as Error).message;
      return new ServiceResponse(
        ResponseStatus.Failed,
        errorMessage,
        null,
  ...
  deleteAddress: async (cust_id: string, address_id: string) => {
    try {
      ...
      const address = await customerRepository.deleteAddress(
        cust_id,
        address_id
      );
      if (address === null) {
  ...
  followQuotation: async (cust_id: string) => {
    try {
      ...
      const errorMessage =
        "Error quotation grand total fetched :" + (ex as Error).message;
      return new ServiceResponse(
        ResponseStatus.Failed,
        errorMessage,
        null,
